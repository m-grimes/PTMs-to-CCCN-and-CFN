...
#Output Files
output_path <- "/Users/karenross/Documents/Bioinformatics/PIR/GrimesGroupCollab/EGFR-Pathway-Crosstalk-Final/"
```
...
#functions used: filter.edges.between, interaction.to.edgeType, edgeType.to.interaction, setCorrEdgeAppearance, extract.gene.names, edgeDprops.RCy32, remove.autophos.RCy3
source(paste(code_path, "Generate_CCCN_CFN.R", sep=""))

#functions used: graph.cfn.cccn2, setNodeColorToRatios2, all.ratio.styles.2, setNodeMapping2, w/o
...
#For each set of interactions, find those where one or both genes had a PTM with a significant change in erlotinib. We will use the same criteria for significantly changed PTMs that we used for the cluster enrichment analysis: a significantly changed PTM is one in which the absolute value of the median of the fold change in the erlotinib vs. control treated samples is at least 2.25 (1.17 on the log2 scale); sites must be observed in at least two experiments. We will do this for the pc9.erl and the all.erl sample groups. Sample groups are defined in cluster-enrichment-final.Rmd. Significantly changed sites for each sample group are listed in sig.site.meds.list.
