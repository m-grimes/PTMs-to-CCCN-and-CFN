...
# Another function to set node size and color to match ratio data. Main difference between this and above is that this one uses the Cytoscape table. 
...
# Funtion to  convert RCy3 to RCy2 names (patch for older functions) 
interaction.to.edgeType <- function(net.edges) {
  if(grepl("source", names(net.edges)[1])) { 
    names(net.edges)[1:2] <- c("Gene.1", "Gene.2") }
  if (any(grepl("interaction", names(net.edges)))) {
    names(net.edges)[grep("interaction", names(net.edges))] <- "edgeType"}
  return(net.edges) }
