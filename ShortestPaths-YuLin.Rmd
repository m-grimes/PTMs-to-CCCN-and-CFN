...
interesting_pairs <- data.frame()
colnames(interesting_pairs) <- colnames(interesting_pairs)

interestin_l <- list()
for(i in nrow(onco_TSG.gzalltgene.physical.cfn)){
  if((gz.cf[which(gz.cf$Gene.Name == onco_TSG.gzalltgene.physical.cfn[i,]$Gene.1),]$H3122SEPTM.C2.ratio)*
      (gz.cf[which(gz.cf$Gene.Name == onco_TSG.gzalltgene.physical.cfn[i,]$Gene.2),]$H3122SEPTM.C2.ratio)<0){
    print(i)
    append(interestin_l,i)
  }
}
