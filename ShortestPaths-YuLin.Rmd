...
# ex_H3122_ALK_MET_SEPTM_all_CFN <- graph.cfn.cccn (ex_H3122_ALK_MET_SEPTM_all, ld=FALSE, gz=TRUE, only.cfn=TRUE)
...
Genral heatmap of genes vs cell line
```{r}
library(reshape2)

experiments_c <- c(colnames(gz.cf)[17:41])

#create DF1:DF25

for (i in 1:25){
  assign(paste("DF",i,sep = ""),data.frame("Genes" = gz.cf$Gene.Name,
                                            "Cells" = replicate(9701,experiments_c[i]),
                                            "Correlations" = gz.cf[17:41][i]))
}

# c("Genes","Cells","Correlations")
gene_cell_ls <- list(DF1,DF2,DF3,DF4,DF5,DF6,DF7,DF8,DF9,DF10,
                     DF11,DF12,DF13,DF14,DF15,DF16,DF17,DF18,DF19,DF20,
                     DF21,DF22,DF23,DF24,DF25)

for (i in gene_cell_ls){
  print(colnames(i))
  colnames(i) <- c("Genes","Cells","Correlations")
  print(colnames(i))
}


# p <- ggplot(gene_cell_df, aes(x=Genes,y=Cells))
# p
