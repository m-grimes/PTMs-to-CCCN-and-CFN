...
options(stringsAsFactors=FALSE)
options(stringsAsFactors=FALSE)
data_path <- "D:/Shortest_path_network/RDataFiles-20200721T045131Z-001/RDataFiles/"
code_path <- "D:/Shortest_path_network/PTMs-to-CCCN-and-CFN/"
...
# This chunck of code selects those PTM sites with at least 2.25-fold change
# and generate the shortest path according to (Drug to target) relationship
```{r}

# cell line = H3122, ExpSet = SEPTM, Drug = Crizotinib, rep = 3
ss1 <- select.sites.w.min.value(gz.cf, "H3122SEPTM.C1.ratio", min_value = 2.25)
paths_1 <- shortest.paths.to.target(ss1, target = c("ALK"))
...
# To obtain a general figure of how different the networks are, Jaccard Method - N(union)/N(intersection) is used to compare the network size difference
# -- Number of EDGES in the Union network / Number of nodes in the intesection network
```{r}

# In the sample order, only those 15 samples treated with - Crizotinib, Erlotinib and Afatinib- are included

sample_order_l <- c(1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 13, 14, 15, 16, 25)


# Uni_i_j <- my.union(paths_i,paths_j)
# This creates the union network of two pathways (pathway i) U (pathway j)
...
# The Ratio_i_j = nrow(Inters_i_j)/nrow(Uni_i_j)
for (i in sample_order_l){
  for (j in sample_order_l){
...
# 15*15 ratio matrix, which aligns the pair-wise Jaccard result of each samples
...
# A more commonly used heatmap package ggplot2 may be applied as well
# However, due to the lack of understanding of ggplot, the pheatmap presents a better result
gp_ratio_dataframe <- data.frame(matrix(nrow = 15, ncol = 16))
gp_ratio_dataframe[,2:16] <- ratio_dataframe
colnames(gp_ratio_dataframe)[2:16] <- colnames(ratio_dataframe)
gp_ratio_dataframe[,1] <- colnames(ratio_dataframe)
melted_gp_ratio_htmp <- melt(gp_ratio_dataframe)
plot1 <- ggplot(melted_gp_ratio_htmp,aes(X1,variable, fill=value)) + geom_tile()
plot1



```
# mody_ss: from line 371 to line
# In the mody version, the selected sites remains same
# the target of shortest pathway is more complete according to the Drug Bank data
# paths_i are overwritten, therefore, the shortest path network is targeting multiple targets
```{r}
# cell line = H3122, ExpSet = SEPTM, Drug = Crizotinib, rep = 3

ss1 <- select.sites.w.min.value(gz.cf, "H3122SEPTM.C1.ratio", min_value = 2.25)
paths_1 <- shortest.paths.to.target(ss1, target = c("ALK","MET","ROS1"))

ss2 <- select.sites.w.min.value(gz.cf, "H3122SEPTM.C2.ratio", min_value = 2.25)
paths_2 <- shortest.paths.to.target(ss2, target = c("ALK","MET","ROS1"))

ss3 <- select.sites.w.min.value(gz.cf, "H3122SEPTM.C3.ratio", min_value = 2.25)
paths_3 <- shortest.paths.to.target(ss3, target = c("ALK","MET","ROS1"))


# cell line = H3122, ExpSet = TenCell, Drug = Crizotinib, rep = 2

ss4 <- select.sites.w.min.value(gz.cf, "H3122_Crizotinib.1Ratio", min_value = 2.25)
paths_4 <- shortest.paths.to.target(ss4, target = c("ALK","MET","ROS1"))

ss5 <- select.sites.w.min.value(gz.cf, "H3122CrizotinibRatio", min_value = 2.25)
paths_5 <- shortest.paths.to.target(ss5, target = c("ALK","MET","ROS1"))
...
ss8 <- select.sites.w.min.value(gz.cf, "STE.1_CrizotinibRatio", min_value = 2.25)
paths_8 <- shortest.paths.to.target(ss8, target = c("ALK","MET","ROS1"))


# cell line = H2228, ExpSet = TenCell, Drug = Crizotinib, rep = 1

ss9 <- select.sites.w.min.value(gz.cf, "H2228CrizotinibRatio", min_value = 2.25)
paths_9 <- shortest.paths.to.target(ss9, target = c("ALK","MET","ROS1"))
...
ss25 <- select.sites.w.min.value(gz.cf, "H1781_AfatinibRatio", min_value = 2.25)
paths_25 <- shortest.paths.to.target(ss25, target = c("EGFR","ERBB2"))
...
gz_exp_15_df <- gz_experiments_df[sample_order_l,]
gz_exp_15_df <- gz_exp_15_df[,2:5]
target_l <- c("ALK_MET_ROS1","ALK_MET_ROS1","ALK_MET_ROS1","ALK_MET_ROS1","ALK_MET_ROS1","ALK_MET_ROS1","ALK_MET_ROS1",
              "EGFR","EGFR","EGFR","EGFR","EGFR","EGFR","EGFR","EGFR_ERBB2")
gz_exp_15_df$target <- target_l
gz_exp_15_df$merge <- paste(gz_exp_15_df$CellLine,gz_exp_15_df$ExpSet,gz_exp_15_df$Drug,gz_exp_15_df$target,gz_exp_15_df$Rep,sep = "_")
...
ann_colors = list(Cell = c(H3122="#99CC66",STE.1="#CCCC99",H2228="#FFFF99",PC9="#87CEFA",HCC4006="#7FFFD4",HCC827="#FF6666",H1781="#FFD700"),Drug = c(Crizotinib="#99CCCC",Erlotinib="#FF9999",Afatinib="#FFCC99"),Target = c(ALK_MET_ROS1="#FFFFCC",EGFR="#CCCCFF",EGFR_ERBB2="#CCFFFF"))
annotation<-data.frame(Cell,Drug,Target)
rownames(annotation) <- rownames(ratio_dataframe)
...
# This might be a repetitive function, since I recall in memory there are function with similar function in other files
# However, I cannot find the function and have to re-invent the wheel
```{r}
...
# Here, I try to make pair-wise comparison heatmap in terms of nodes number, other than edge number( which is done)
```{r}
# Node count list for the unions NLU, Node count list for intersection NLI
# The node count refers to node number the pairwise union/intersection network
NLU <- list()
NLI <- list()
for (i in sample_order_l){
  for (j in sample_order_l){
...
# The node-count-based analysis consistent with the edge-count-based result.
# However, the result of node-count exhibit less patterned result
```{r}
NLU <- as.numeric(NLU)
NLI <- as.numeric(NLI)
...
pheatmap::pheatmap(node_ratio_df, annotation= annotation,angle_col ="315")

```


# The select.sites.w.min.value function select those sites with either upregulation or downregulation.
# To specify the fold change orientation, select.sites.up.regulated and select.sites.down.regulated functions are created
...
# modifed version of select sites function that select nodes are downregulated only
select.sites.down.regulated <- function(df, exp, min_value = 2.25) {
  data <- df[, exp]
  df <- df[ which((data*-1) > log2(min_value)), ]
  return(df)
}

# In oreder to organize the samples into a more reasonable order, sample_names are arranged in such a way according to
# cell line and Drug 
...
# shortest path analysis of 15 samples on up-regulated genes
# up_paths_i <- shortest.paths.to.target(up_ss_i,target= x)
# recall that the target_l is corresponding to 15 samples
...
#This is a summary table of nodes and edge number in the shortest path network

# edge_l and node_L counts the up-regulated sites only
edge_l <- list()
for (i in sample_order_l){
...
# While total_edge_l and total_node_l count both up-regulated and down regulated sites
total_edge_l <- list()
total_node_l <- list()
...
sample_counting <- data.frame(edge_l,node_l,sample_order_l,total_edge_l,total_node_l,gz_exp_15_df$merge)
...
# This chunck of code focuse only on those genes with 2.25-fold upregulated genes
```{r}
...
# graph.cfn.cccn (ex_H3122_ALK_SEPTM, ld=FALSE, gz=TRUE, only.cfn=TRUE)
# all.ratio.styles()
# graph.cfn.cccn (ex_H3122_ALK_SEPTM, ld=FALSE, gz=TRUE, only.cfn=FALSE)
# all.ratio.styles()
...
# H3122_ALK_MET_SEPTM 
ALK_MET_ex_MYH9_up_paths_1 <- shortest.paths.to.target(up_ss_1,target = c("ALK","MET","ROS1"),excluded_gene = "MYH9")
ALK_MET_ex_MYH9_up_paths_2 <- shortest.paths.to.target(up_ss_2,target = c("ALK","MET","ROS1"),excluded_gene = "MYH9")
ALK_MET_ex_MYH9_up_paths_3 <- shortest.paths.to.target(up_ss_3,target = c("ALK","MET","ROS1"),excluded_gene = "MYH9")
...
# ex_MYH9_H3122_ALK_MET_SEPTM_CFN <- graph.cfn.cccn (ex_MYH9_H3122_ALK_MET_SEPTM, ld=FALSE, gz=TRUE, only.cfn=TRUE,pruned = FALSE)
# setCorrEdgeAppearance(ex_MYH9_H3122_ALK_MET_SEPTM_CFN)
# FixEdgeDprops.RCy32()
...
#  chunck of code focuse only on those genes with 2.25-fold change genes
```{r}
# Gene presence count/ how many edges are connected to this gene
# It counts the occurrence of each gene in a shortest path network in an ordered table
...
# my.outersect
# This function finds those genes that only has occurence of 1, indicating it can be the most down-stream part
...
# ex_H3122_ALK_MET_SEPTM_all_CFN <- graph.cfn.cccn (ex_H3122_ALK_MET_SEPTM_all, ld=FALSE, gz=TRUE, only.cfn=FALSE)
...
# onco_c = collection of oncogenes (dual genes included)
# TSG_c = collection of tumor suppressor gens (dual genes included)
# dual_c = collection of those genes can be consider as both oncogene and tumor suppresor genes
...
colnames(onco_TSG.gzalltgene.physical.cfn)<- colnames(ex_H3122_ALK_MET_SEPTM_all)

# cancer_ex_H3122_ALK_MET_SEPTM_all_CFN <- graph.cfn.cccn (onco_TSG.gzalltgene.physical.cfn, only.cfn=FALSE,pruned = FALSE)
...
# in order to have a easier interpretation of the gz.cf file, I create a gz.cf_mody1, in the gz.cf_mody1, only the summary gene name (no PTM sites) would be included, and the cell line correlation would be the highest number among the column (same Gene.Name). 
...
for (gene in unique_gene_gz.cf_Gene.Name){
  ...
  if (nrow(same_gene_sub_gz.cf) == 1){
    gz.cf_mod_row <- same_gene_sub_gz.cf[1,]
  }
  if (nrow(same_gene_sub_gz.cf) >1){
  cell_correlation <- gensvm.maxabs.scale(same_gene_sub_gz.cf[,17:41])
  gz.cf_mod_row <- same_gene_sub_gz.cf[1,]
...
non_zero_gz.cf_mody1 <- gz.cf_mody1[which(rowSums(gz.cf_mody1[17:41]) != 0),]

non_zero_gz.cf_mody2 <- non_zero_gz.cf_mody1

non_zero_gz.cf_mody2[,17:25] <- non_zero_gz.cf_mody1[,c(17,18,19,32,33,36,37,41,30)]#criz
colnames(non_zero_gz.cf_mody2[,17:25]) <- colnames(non_zero_gz.cf_mody1[,c(17,18,19,32,33,36,37,41,30)])
non_zero_gz.cf_mody2[,26:32] <- non_zero_gz.cf_mody1[,c(23,24,25,39,40,35,38)]#erl
colnames(non_zero_gz.cf_mody2[,26:32]) <- colnames(non_zero_gz.cf_mody1[,c(23,24,25,39,40,35,38)])
non_zero_gz.cf_mody2[,33:38] <- non_zero_gz.cf_mody1[,c(20,21,22,26,27,28)]#PR171
colnames(non_zero_gz.cf_mody2[,33:38]) <- colnames(non_zero_gz.cf_mody1[,c(20,21,22,26,27,28)])
non_zero_gz.cf_mody2[,39:40] <- non_zero_gz.cf_mody1[,c(31,34)]#disa
colnames(non_zero_gz.cf_mody2[,39:40]) <- colnames(non_zero_gz.cf_mody1[,c(31,34)])
non_zero_gz.cf_mody2[,41] <- non_zero_gz.cf_mody1[,39]#afat
colnames(non_zero_gz.cf_mody2[,41]) <- colnames(non_zero_gz.cf_mody1[,39])
non_zero_gz.cf_mody1 <- non_zero_gz.cf_mody2
...
```{r}
#nrow(onco_TSG.gzalltgene.physical.cfn) c(1:369,372:460,463:853,860:922)
interesting_rows <- c()
for(i in 1:c(1:369,372:460,463:853,860:922)){
...
f1<- onco_TSG.gzalltgene.physical.cfn[interesting_rows,]
colnames(f1) <- colnames(ex_H3122_ALK_MET_SEPTM_all)
# graph.f1 <- graph.cfn.cccn(f1, ld=FALSE, gz=TRUE, only.cfn=TRUE,pruned = FALSE)
```
...
#cancer_gz.cf
```{r}
cancer_gz.cf <- non_zero_gz.cf_mody1[which(non_zero_gz.cf_mody1$cancer == 1),]
data_1 <- cancer_gz.cf[,c(2,17:41)]
rownames(data_1) <- data_1$Gene.Name
data_1 <- data_1[2:26]
data_1 <- as.matrix(data_1)
temp_data_1 <- data_1[,c(1,2,3,16,17,20,21,25,14,7,8,9,23,24,19,22,4,5,6,10,11,12,15,18,23)]
# data_1[,1:9] <- temp_data_1[,c(1,2,3,16,17,20,21,25,14)]
# data_1[,10:16] <- temp_data_1[,c(7,8,9,23,24,19,22)]
# data_1[,17:22] <- temp_data_1[,c(4,5,6,10,11,12)]
# data_1[,23:24] <- temp_data_1[,c(15,18)]
# data_1[,25] <- temp_data_1[,c(23)]

pheatmap::pheatmap(t(temp_data_1),cluster_rows=FALSE)

```

# TSG_gz.cf
```{r}
TSG_gz.cf <- non_zero_gz.cf_mody1[which(non_zero_gz.cf_mody1$cancer == 2),]
data_2 <- TSG_gz.cf[,c(2,17:41)]
rownames(data_2) <- data_2$Gene.Name
data_2 <- data_2[2:26]
data_2 <- as.matrix(data_2)
temp_data_2 <- data_2[,c(1,2,3,16,17,20,21,25,14,7,8,9,23,24,19,22,4,5,6,10,11,12,15,18,23)]
pheatmap::pheatmap(t(temp_data_2),cluster_rows=FALSE)

```

#dual_gz.cf
```{r}
dual_gz.cf <- non_zero_gz.cf_mody1[which(non_zero_gz.cf_mody1$cancer == 3),]
data_3 <- dual_gz.cf[,c(2,17:41)]
rownames(data_3) <- data_3$Gene.Name
data_3 <- data_3[2:26]
data_3 <- as.matrix(data_3)
heatmap(t(data_3))

pheatmap::pheatmap(t(data_3),cluster_rows=FALSE)
```


How about the targets of the drugs
```{r}
ALk.criz.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "ALK"),][-1,]
MET.criz.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "MET"),][-1,]
EGFR.erl.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "EGFR"),][-1,]
ABL1.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "ABL1"),][-1,]
SRC.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "SRC"),][-1,]
EPHA2.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "EPHA2"),][-1,]
LCK.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "LCK"),][-1,]
YES1.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "YES1"),][-1,]
KIT.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "KIT"),][-1,]
STAT5B.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "STAT5B"),][-1,]
FYN.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "FYN"),][-1,] # FGR is somehow included
TEC.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "TEC"),][-1,] # The target is supposed to be BTK, however, TEC seems to have the BTK domain
BCR.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "BCR"),][-1,]
CSK.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "CSK"),][-1,]
EPHA3.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "EPHA3"),][-1,] # suppose to be EPHA5
EPHB4.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "EPHB4"),][-1,]
FRK.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "FRK"),][-1,]
HSPA1B.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "HSPA1B"),][-1,]
LYN.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "LYN"),][-1,]
MAPK14.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "MAPK14"),][-1,]
PPAT.dis.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "PPAT"),][-1,]
ERBB2.afa.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "ERBB2"),][-1,]
ERBB3.afa.gz.cf <- gz.cf[which(gz.cf$Gene.Name == "ERBB3"),][-1,] # suppose to be ERBB4, but only ERBB2/3 are available, so

target.related.PTMs.gz.cf <- rbind(ALk.criz.gz.cf,
MET.criz.gz.cf,
EGFR.erl.gz.cf,
ABL1.dis.gz.cf,
SRC.dis.gz.cf,
...
data_4 <- target.related.PTMs.gz.cf[,c(1,17:41)]
rownames(data_4) <- data_4$id
data_4 <- data_4[2:26]
data_4 <- as.matrix(data_4)
data_4 <- data_4[which(rowSums(is.na(data_4)) < 21),]
temp_data_4 <- data_4[,c(1,2,3,16,17,20,21,25,14,7,8,9,23,24,19,22,4,5,6,10,11,12,15,18,13)]


Cell.Line <- c("H3122","H3122","H3122","H3122","H3122","HCC78","HCC78","SET.1","H2228","PC9","PC9","PC9","PC9","PC9","HCC4006","HCC827","H3122","H3122","H3122","PC9","PC9","PC9","H2286","H366","H1781")
Exp.set <- c("SEPTM","SEPTM","SEPTM","TenCell","TenCell","TenCell","TenCell","TenCell","TenCell","SEPTM","SEPTM","SEPTM","TenCell","TenCell","TenCell","TenCell","SEPTM","SEPTM","SEPTM","SEPTM","SEPTM","SEPTM","TenCell","TenCell","TenCell")
DRUG <- c("Crizotinib","Crizotinib","Crizotinib","Crizotinib","Crizotinib","Crizotinib","Crizotinib","Crizotinib","Crizotinib","Erlotinib","Erlotinib","Erlotinib","Erlotinib","Erlotinib","Erlotinib","Erlotinib","PRP171","PRP171","PRP171","PRP171","PRP171","PRP171","Disatinib","Disatinib","Afatinib")
REP <- c("1","2","3","1","2","1","2","1","1","1","2","3","1","2","1","1","1","2","3","1","2","3","1","1","1")

sample_25.df <- data.frame(Cell.Line,Exp.set,DRUG,REP)
sample_25.df$merge <- paste(sample_25.df$Cell.Line,sample_25.df$Exp.set,sample_25.df$DRUG,sample_25.df$REP, sep = "_")
colnames(temp_data_4) <- sample_25.df$merge

CELL <- factor(Cell.Line, levels = unique(Cell.Line))
Exp.annotation <- factor(Exp.set,levels = unique(Exp.set))
DRUG <- factor(DRUG,levels = unique(DRUG))
Annotation <- data.frame(CELL,DRUG)
rownames(Annotation) <- colnames(temp_data_4)


GENE <- c("ALK","ALK","MET","MET",
          "MET","MET","MET","MET",
          "MET","EGFR","EGFR","EGFR",
          "EGFR","EGFR","EGFR","EGFR",
          "EGFR","EGFR","EGFR","ABL1",
          "ABL1","EPHA2","EPHA2","EPHA2",
          ...
          "FRK","FRK","FRK","HSPA8",
          "HSPA8","HSPA8","HSPA8","HSPA8",
          "HSPA8","HSPA8","HSPA8","HSPA8",
          "HSPA8","HSPA8","LYN","LYN",
          "LYN","LYN","LYN","MAPK14",
          "MAPK14","PPAT","ERBB2","ERBB3",
          "ERBB3")
PTM.GENE <- factor(GENE, levels = unique(GENE))
GENE.DRUG <- c("Crizotinib","Crizotinib","Crizotinib","Crizotinib",
          "Crizotinib","Crizotinib","Crizotinib","Crizotinib",
          "Crizotinib","Erlotinib&Disatinib","Erlotinib&Disatinib","Erlotinib&Disatinib",
          "Erlotinib&Disatinib","Erlotinib&Disatinib","Erlotinib&Disatinib","Erlotinib&Disatinib",
          "Erlotinib&Disatinib","Erlotinib&Disatinib","Erlotinib&Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          ...
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Disatinib",
          "Disatinib","Disatinib","Disatinib","Afatinib",
          "Afatinib")
GENE_DRUG <- factor(GENE.DRUG,levels = unique(GENE.DRUG))
Annotation_row <- data.frame(PTM.GENE,GENE.DRUG)
rownames(Annotation_row) <- rownames(temp_data_4)
          
          
          
gene.annotation <- data.frame(GENE,rownames(temp_data_4))


pheatmap::pheatmap(t(temp_data_4),cluster_rows=FALSE, cluster_cols = FALSE,angle_col ="315",na_col = "black",annotation_row = Annotation,annotation_col = Annotation_row)
...
```{r}
gz.cf.NA <- gz.cf
gz.cf.NA[17:41][gz.cf.NA[17:41] == 0] <- NA
gz.cf <- gz.cf.NA
```
...
enzymes
```{r}
enzyme_table <- read.csv('Enzymes_Human.csv')
enzyme_names <- enzyme_table$Gene.names...primary..

gz.cf_enz <- gz.cf[gz.cf$Gene.Name %in% enzyme_names,]
gz.cf[gz.cf$cancer == 1,]

# gz.cf$interested.enzymes <- 0
# gz.cf[which(gz.cf$Gene.Name %in% enzyme_names),]$interested.enzymes <- 1
# interested.enzyme_gz.cf <- gz.cf[which(gz.cf$interested.enzymes==1),]
# data_6 <- interested.enzyme_gz.cf[,c(1,17:41)]
# rownames(data_6) <- data_6$id
# data_6 <- data_6[2:26]
# temp_data_6 <- data_6[,c(1,2,3,16,17,20,21,25,14,7,8,9,23,24,19,22,4,5,6,10,11,12,15,18,13)]
# data_6 <- temp_data_6
# data_6 <- data_6[rowSums(is.na(data_6)) < 13,]
# pheatmap::pheatmap(t(data_6),cluster_rows=FALSE,cluster_cols = FALSE,angle_col ="315")
...
cancer.related.genes <- unique(c(onco_c,TSG_c))

cancer.related.enzymes <- c()
for (i in enzyme_names){
  if (i %in% cancer.related.genes){
    cancer.related.enzymes <- c(cancer.related.enzymes,i)
  }
}

gz.cf$cancer.enzyme <- 0
gz.cf[which(gz.cf$Gene.Name %in% cancer.related.enzymes),]$cancer.enzyme <- 1

enzymes_gz.cf <- gz.cf[which(gz.cf$cancer.enzyme == 1),]
data_5 <- enzymes_gz.cf[,c(1,17:41)]
rownames(data_5) <- data_5$id
data_5 <- data_5[2:26]
temp_data_5 <- data_5[,c(1,2,3,16,17,20,21,25,14,7,8,9,23,24,19,22,4,5,6,10,11,12,15,18,13)]
data_5 <- temp_data_5
data_5 <- data_5[rowSums(is.na(data_5)) < 13,]


pheatmap::pheatmap(t(data_5),cluster_rows=FALSE,cluster_cols = FALSE,angle_col ="315")
...
graph<-graph_from_data_frame(example2, directed = T, vertices = NULL)
...
ll <- CorenessLayout(graph);

# plot

plot(graph, layout=ll, vertex.size=15, vertex.color=colbar[coreness], vertex.frame.color=colbar[coreness], main='Coreness')
```
...
nrow(gz_H3_CRI_keep2_paths)da
# changed_genes_network <- graph.cfn.cccn (gz_pc9_erl_keep2_paths, ld=FALSE, gz=TRUE, only.cfn=TRUE)
# all.ratio.styles()

```

# t-test of PC9 Erlotinib groups(SEPTM/Tencell) to see if the fold change is consistent

```{r}

gz.cf_pc9_erl <- gz.cf[c("PC9SEPTM.E1.ratio","PC9SEPTM.E2.ratio","PC9SEPTM.E3.ratio","PC9_Erlotinib.1Ratio","PC9_ErlotinibRatio")]
rownames(gz.cf_pc9_erl) <- t(gz.cf["id"])

gz.cf_pc9_erl <- gz.cf_pc9_erl[(rowSums(is.na(gz.cf_pc9_erl[1:3]))<2),]
gz.cf_pc9_erl <- gz.cf_pc9_erl[(rowSums(is.na(gz.cf_pc9_erl[4:5]))<1),]

gz.cf_pc9_erl$pValue_c <- NA
gz.cf_pc9_erl$pValue <- NA

for (i in 1:460){
  r <- gz.cf_pc9_erl[i,]
  SEPTM <- as.vector(r[1:3])
  TEN <- as.vector(r[4:5])
  a <- t.test(SEPTM,TEN)
  p1 <- as.numeric(a$p.value)
  #b <- t.test(SEPTM)
  p2 <- as.numeric(b$p.value)
  gz.cf_pc9_erl[i,"pValue_c"] <- p1
  #gz.cf_pc9_erl[i,"pValue"] <- p2
}

gz.cf_pc9_erl[gz.cf_pc9_erl$pValue_c<0.05,]
```
